...
#DATABASE = '/var/www/HackDay/people.db'
...
def peopleDB():
    ...
    stats = query_db('select profile, round(avg(klout_score)), round(avg(twitter_followers)), round(avg(importance)) from people group by profile')
    
    str_query = 'select * from people where profile = \'NewsCred\' and importance > 300 and (last_seen like \'%' + time.strftime("%d %b %Y", time.gmtime()) + '%\' OR last_seen like \'%' + time.strftime("%d %b %Y", time.gmtime(time.time() - 24*60*60)) + '%\') and organization <> \'\'order by importance desc'
    print str_query
    int_people = query_db(str_query)

    str_query = 'select profile, sum(twitter_followers) from people group by profile'
    print str_query
    followers = query_db(str_query)

    return render_template('peopleDB.html', stats = stats, int_people = int_people, followers=followers)


@app.route('/<profile>/people/')
def allpeople(profile):
...
@app.route('/<profile>/people/recent/')
def recentpeople(profile):
...
@app.route('/<profile>/people/important/')
def importantpeople(profile):

    query_str = 'select * from people where importance > 150  and organization <> \'\' and profile = \'' + profile + '\' order by importance desc'
    print query_str
    important_people = query_db(query_str)
    return render_template('people.html', important_people=important_people)

@app.route('/<profile>/people/engaged/')
def engagedpeople(profile):

    query_str = 'select p.*, count(c.twitter_id) from content c, people p where c.twitter_id = p.twitter_id and c.profile = \'' + profile + '\'  group by c.twitter_id order by count(c.twitter_id) desc'
    print query_str
    engaged_people = query_db(query_str)
    return render_template('people.html', engaged_people=engaged_people)


@app.route('/<profile>/person/<twitter_id>/')
def person(profile, twitter_id):

    query_str = 'select * from people where twitter_id = \'' + twitter_id + '\' and profile = \'' + profile + '\''
    print query_str
    person_result = query_db(query_str)

    query_str = 'select * from content where twitter_id = \'' + twitter_id + '\' and profile = \'' + profile + '\''
    print query_str
    journey_result = query_db(query_str)

    return render_template('person.html', person_result=person_result, journey_result = journey_result)
